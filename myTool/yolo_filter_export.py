...
def has_class_in_label(label_path: str, class_id: str) -> bool:
    """檢查 YOLO 標註檔是否包含指定類別（比對第一欄 class id）。"""
    try:
...
def scan_and_export(
    ...
    """
    遞迴掃描 dataset_root，找出含有 `class_id` 的標註檔，並把該標註與對應影像複製到 output_root。
    ...
    for root, _, files in os.walk(dataset_root):
        for fname in files:
            ...
            if not has_class_in_label(label_path, class_id):
                continue
    ...
    print(f"符合類別 {class_id} 的標註：{matched}")
...
def main():
    parser = argparse.ArgumentParser(
        description="篩出含指定類別的 YOLO 影像與標註，並另存到指定目錄（保留相對路徑結構）。"
    )
    parser.add_argument("-s", "--src", required=True, help="資料集根目錄（會被遞迴掃描）")
    parser.add_argument("-c", "--class-id", required=True, help="要篩選的類別編號（例如 '8'）")
    parser.add_argument("-o", "--out", required=True, help="輸出根目錄")
    parser.add_argument("-v", "--verbose", action="store_true", help="詳細模式（目前未使用）")
